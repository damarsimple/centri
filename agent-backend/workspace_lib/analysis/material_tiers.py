...
SYSTEM_TMPL = (
    ...
    "8d. ONE MOMENT PER SENTENCE. The period (seconds for one turn) and the turn rate (turns "
    "each second) are the SAME fact stated two ways — one is 1 divided by the other. On a clip "
    "that speeds up or slows down they differ from moment to moment, so NEVER put a lap time "
    "from one moment beside a turn rate from another. Above all, never introduce a rate with "
    "'that'/'this' turn rate right after a lap time it does not match: writing 'it completes a "
    "full turn in 2.07 seconds. That turn rate is about 0.30 full turns each second' tells the "
    "reader one thing and its opposite (0.30 turns each second IS a 3.34-second lap). Either "
    "quote the pair from the SAME moment, or say in words which moment each belongs to ('right "
    "after the flick a lap takes 2.07 seconds; averaged over the whole clip it makes about "
    "0.30 full turns each second').\n"
