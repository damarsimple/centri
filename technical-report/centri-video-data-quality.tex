...
angle-dependent box quality, and a trust channel that no longer infers health from the absence of a
diagnosis}}
...
What \emph{is} demonstrable is that \textbf{the detector's box depends on the object's angle}. The
...
hub offset, but carrying an unexplained angle-locked artifact and too short to characterise it. The
pipeline now says so rather than inferring health from the absence of a diagnosis
(\S\ref{sec:trustchannel}).
...
\begin{itemize}
  ...
  every mechanism we can test (\S\ref{sec:tt3}), while its detector's box demonstrably varies with
  the object's angle ($432 \to 470$\,px, $44\%$ phase-locked). Until these are understood we cannot
  claim to know what those clips' per-instant data should be --- and the corpus's angle-dependent box
  quality is now a known defect of the tracker, not only of the footage.
...
\begin{enumerate}
  ...
  marker extent is now the binding constraint (\S\ref{sec:marker}), not projection.
