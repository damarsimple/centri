...
phase-lock statistic, which retracts the claim that \texttt{turntable-3}'s ripple is angle-locked}}
...
\textbf{Verdict: GOLD for time-averages, \und{} per-instant.} Cleared of perspective by the static
hub offset, and now cleared of any tracker-specific defect, but carrying a ripple of $\approx 17\%$
of mean $\omega$ that no mechanism we can test accounts for, over too few revolutions to characterise
(\S\ref{sec:phaselocknull}). The pipeline now says so rather than inferring health from the absence
of a diagnosis (\S\ref{sec:trustchannel}).
...
($r_{\mathrm{fit}}$ $0.254 \rightarrow 0.260$\,m). What follows is what remains.}
...
\texttt{roundabout-4046} (agent-detected axle, no manual intervention); the golden set referenced
throughout is seven clips across four phenomena, all passing their material checks.
