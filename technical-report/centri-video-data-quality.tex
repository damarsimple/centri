...
rather than of the tracker --- and a trust channel that no longer infers health from the absence of a
diagnosis}}
...
$r={+}0.980$ and the mean rates agree to $0.10\%$; \texttt{turntable-1} gives $r={+}0.980$ and
$0.02\%$, \texttt{turntable-2} $r={+}0.991$ and $0.13\%$. The two angular tracks sit $0.46^\circ$
rms apart where the disputed ripple is worth $\approx1.8^\circ$ --- the trackers agree with each
other far more closely than either agrees with a clean coast-down.
...
One further use of the same data: two independent trackers agreeing on mean $\omega$ to
$0.02$--$0.13\%$ is not the external ground truth this report calls its most valuable missing piece
...
hub offset, and now cleared of any tracker-specific defect, but carrying an unexplained angle-locked
artifact and too short to characterise it. The pipeline now says so rather than inferring health
from the absence of a diagnosis (\S\ref{sec:trustchannel}).
...
\begin{itemize}
  ...
  ripple that is neither perspective nor occlusion; \texttt{turntable-3} carries one that survives
  every mechanism we can test (\S\ref{sec:tt3}), while the measured box demonstrably varies with
  the object's angle ($432 \to 470$\,px, $44\%$ phase-locked). Until these are understood we cannot
  claim to know what those clips' per-instant data should be. The ablation of \S\ref{sec:tt3}
  narrows the search without closing it: a second, structurally different detector reproduces the
  ripple, so the defect is not in any particular tracker --- it is in what the footage presents to
  \emph{any} appearance-based tracker.
