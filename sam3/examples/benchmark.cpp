 ...
 * (Metal/CUDA), then prints a formatted comparison table.
 *
 ...
 *   --cpu-only            Skip GPU runs (Metal/CUDA)
...
static BenchWire run_single_benchmark(const std::string & model_path,
    ...
    snprintf(wire.backend, sizeof(wire.backend), "%s", sam3_get_backend_name(*model));
...
static void child_benchmark(const std::string & model_path,
    ...
    ssize_t nwritten = write(write_fd, &wire, sizeof(wire));
    (void) nwritten;
    close(write_fd);
    _exit(wire.ok ? 0 : 1);
}
...
static BenchResult run_benchmark_isolated(const ModelEntry & entry,
    ...
    if (nread == (ssize_t)sizeof(wire) && wire.backend[0] != '\0') {
        res.backend = wire.backend;
    }
...
int main(int argc, char ** argv) {
    ...
    for (size_t i = 0; i < runs.size(); i++) {
        ...
        const char * requested_backend = requested_backend_label(run.use_gpu);

        fprintf(stderr, "[%3zu/%zu] %s (%s) ...\n",
                i + 1, runs.size(), run.entry->name.c_str(), requested_backend);
