...
/* ── Profiling: enabled at runtime via SAM3_PROFILE=1 env var ─────────────
   Zero overhead when disabled (one-time env check cached in static).
   Thread safety: profiling state is NOT thread-safe. All SAM3_PROFILE_*
   calls must come from a single thread. This is fine because sam3.cpp's
   pipeline orchestration is single-threaded (ggml parallelizes internally).
   Usage:
     SAM3_PROFILE_BEGIN(section_name);
     ...
     SAM3_PROFILE_END(section_name);
   At end of a pipeline, call sam3_profile_print_and_reset() to dump table. */

static bool sam3_profiling_enabled() {
    // C++11 guarantees thread-safe initialization of function-local statics.
    static const bool enabled = [] {
        const char* v = getenv("SAM3_PROFILE");
        return v && v[0] == '1';
    }();
    return enabled;
}

struct sam3_profile_entry {
    double total_ms = 0.0;
    int    count    = 0;
    int    order    = 0;  // insertion order for stable output
};

// NOT thread-safe — called only from the main pipeline thread.
static std::map<std::string, sam3_profile_entry>& sam3_profile_map() {
    static std::map<std::string, sam3_profile_entry> m;
    return m;
}

static int& sam3_profile_order_counter() {
    static int c = 0;
    return c;
}

static void sam3_profile_record(const char* name, double ms) {
    auto& m = sam3_profile_map();
    auto it = m.find(name);
    if (it == m.end()) {
        m[name] = {ms, 1, sam3_profile_order_counter()++};
    } else {
...
static void sam3_profile_print_and_reset() {
    auto& m = sam3_profile_map();
    if (m.empty()) return;

    // Sort by insertion order
    std::vector<std::pair<std::string, sam3_profile_entry>> sorted(m.begin(), m.end());
    std::sort(sorted.begin(), sorted.end(),
              [](const auto& a, const auto& b) { return a.second.order < b.second.order; });

    double total = 0;
    for (const auto& e : sorted) total += e.second.total_ms;

    fprintf(stderr, "\n┌─ SAM3 PROFILE ─────────────────────────────────────────────┐\n");
    fprintf(stderr, "│ %-30s │ %8s │ %5s │ %5s │\n", "Section", "Time(ms)", "Count", "  %  ");
    fprintf(stderr, "├────────────────────────────────┼──────────┼───────┼───────┤\n");
    for (const auto& e : sorted) {
        double pct = total > 0 ? 100.0 * e.second.total_ms / total : 0;
        fprintf(stderr, "│ %-30s │ %8.1f │ %5d │ %4.1f%% │\n",
                e.first.c_str(), e.second.total_ms, e.second.count, pct);
    }
    fprintf(stderr, "├────────────────────────────────┼──────────┼───────┼───────┤\n");
    fprintf(stderr, "│ %-30s │ %8.1f │       │       │\n", "TOTAL", total);
    fprintf(stderr, "└────────────────────────────────┴──────────┴───────┴───────┘\n\n");

    m.clear();
    sam3_profile_order_counter() = 0;
}

// Scoped profiler — records time from construction to destruction
struct sam3_profile_scope {
    const char* name;
    int64_t     start_us;
    sam3_profile_scope(const char* n) : name(n), start_us(0) {
        if (sam3_profiling_enabled()) start_us = ggml_time_us();
    }
    ~sam3_profile_scope() {
        if (start_us > 0) {
            double ms = (ggml_time_us() - start_us) / 1000.0;
            sam3_profile_record(name, ms);
        }
    }
};

#define SAM3_PROFILE_SCOPE(name) \
    sam3_profile_scope _sam3_prof_##__LINE__(name)

#define SAM3_PROFILE_BEGIN(name) \
    int64_t _sam3_prof_t_##name = sam3_profiling_enabled() ? ggml_time_us() : 0

#define SAM3_PROFILE_END(name) \
    do { if (_sam3_prof_t_##name > 0) \
        sam3_profile_record(#name, (ggml_time_us() - _sam3_prof_t_##name) / 1000.0); \
    } while (0)

#define SAM3_PROFILE_PRINT() \
    do { if (sam3_profiling_enabled()) sam3_profile_print_and_reset(); } while (0)
...
struct sam3_state {
    ...
    // Cached Hiera positional embedding (computed once, reused across frames)
    // Safe to cache: depends only on model weights (constant) and img_size (constant per state).
    std::vector<float> hiera_pos_embed_cache;
    int hiera_pos_embed_cache_size = 0;  // img_size that was used; 0 = not cached
...
// ── CUDA-safe flash attention wrapper ─────────────────────────────────────────
// CUDA flash attention only supports head dims in {40,64,72,80,96,112,128,256,576}.
// For unsupported head dims (e.g. 32), fall back to manual Q·K^T → scale → softmax → V.
// On CPU/Metal, flash attention supports all dims so we always use it.
static struct ggml_tensor* sam3_flash_attn_or_sdpa(
    struct ggml_context* ctx,
    struct ggml_tensor* Q,   // [HD, N_q, NH, B]
    struct ggml_tensor* K,   // [HD, N_kv, NH, B]
    struct ggml_tensor* V,   // [HD, N_kv, NH, B]
    struct ggml_tensor* mask, // or nullptr
    float scale) {
    const int64_t HD = Q->ne[0];

#ifdef GGML_USE_CUDA
    // Check if head dim is supported by CUDA flash attention
    bool cuda_fa_ok = false;
    switch (HD) {
        case 40: case 64: case 72: case 80: case 96:
        case 112: case 128: case 256: case 576:
            cuda_fa_ok = true; break;
        default:
            cuda_fa_ok = false; break;
    }
    if (!cuda_fa_ok) {
        // Manual SDPA fallback: Q·K^T → scale → softmax → V
        const int64_t N_q  = Q->ne[1];
        const int64_t NH   = Q->ne[2];
        const int64_t B    = Q->ne[3];
        const int64_t N_kv = K->ne[1];

        // Flatten NH*B for batched matmul
        auto* Q3 = ggml_reshape_3d(ctx, Q, HD, N_q, NH * B);
        auto* K3 = ggml_reshape_3d(ctx, ggml_cont(ctx, K), HD, N_kv, NH * B);
        auto* V3 = ggml_reshape_3d(ctx, ggml_cont(ctx, V), HD, N_kv, NH * B);

        // attn_scores = K^T @ Q → [N_kv, N_q, NH*B]
        auto* scores = ggml_mul_mat(ctx, K3, Q3);
        ...
        // out = V @ attn → [HD, N_q, NH*B]
        auto* VT = ggml_cont(ctx, ggml_permute(ctx, V3, 1, 0, 2, 3)); // [N_kv, HD, NH*B]
        auto* out = ggml_mul_mat(ctx, VT, scores); // [HD, N_q, NH*B]

        // Reshape to [HD, NH, N_q, B] to match flash_attn_ext output layout
        out = ggml_reshape_4d(ctx, out, HD, N_q, NH, B);
...
const char * sam3_get_backend_name(const sam3_model & model) {
...
static struct ggml_tensor* sam3_vit_block_forward(struct ggml_context* ctx,
...

    {
        ...
        auto* attn_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
...
static struct ggml_tensor* sam3_text_block_forward(struct ggml_context* ctx,
    ...
    float scale = 1.0f / sqrtf((float)HD);
    auto* attn_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, causal_mask, scale);
...
static struct ggml_tensor* sam3_build_text_encoder_graph(struct ggml_context* ctx,
    ...
    x = ggml_cont(ctx, x);  // ensure contiguous for CUDA broadcast add
...
static bool edgetam_encode_image(sam3_state& state,
    ...
    // ── Allocate + compute ───────────────────────────────────────────────
    SAM3_PROFILE_BEGIN(graph_alloc);
    ...
    SAM3_PROFILE_END(graph_alloc);

    // Set input image
    SAM3_PROFILE_BEGIN(data_upload);
    ...
    // Copy FPN outputs to state (device-to-device when on GPU)
    for (int i = 0; i < n_fpn; ++i) {
...
static struct ggml_tensor* edgetam_perceiver_layer_forward(
    ...
    // ── Cross-attention: latents attend to features ─────────────────────
    {
        ...
        auto* attn = sam3_flash_attn_or_sdpa(ctx, q, k, v, nullptr, scale);
...
static bool sam2_encode_image_hiera(sam3_state& state,
    ...
    // ── Preprocess ───────────────────────────────────────────────────────
    SAM3_PROFILE_BEGIN(preprocess);
    auto img_data = sam2_preprocess_image(image, img_size);
    SAM3_PROFILE_END(preprocess);
    ...
    // Set positional embedding — cached across frames
    // Safe: depends only on model weights (constant) and img_size (constant per state).
    // Invalidate if img_size ever changes (shouldn't happen in normal usage).
    {
        auto* pe_tensor = ggml_graph_get_tensor(graph, "hiera_pos_embed");
        if (state.hiera_pos_embed_cache_size != img_size) {
            int pe_H = img_size / 4;
            int pe_W = img_size / 4;
            state.hiera_pos_embed_cache = sam2_compute_pos_embed(model, pe_H, pe_W);
            state.hiera_pos_embed_cache_size = img_size;
        }
        ggml_backend_tensor_set(pe_tensor, state.hiera_pos_embed_cache.data(), 0,
                                state.hiera_pos_embed_cache.size() * sizeof(float));
    }

    SAM3_PROFILE_END(data_upload);

    // Compute
    SAM3_PROFILE_BEGIN(backbone_compute);
    ...
    SAM3_PROFILE_END(backbone_compute);

    // ── Copy results to state ────────────────────────────────────────────
    SAM3_PROFILE_BEGIN(data_readback);
    // Free old state buffers (but NOT pe_buf/pe_ctx — those are cached across frames)
    if (state.buffer) { ggml_backend_buffer_free(state.buffer); state.buffer = nullptr; }
    if (state.ctx) { ggml_free(state.ctx); state.ctx = nullptr; }
    ...
    // Compute sinusoidal PE for each FPN level — cache across frames
    // PE depends only on spatial dims + neck_dim, which are constant per model.
    if (!state.pe_buf) {
        size_t pe_ctx_size = ggml_tensor_overhead() * 16;
        struct ggml_init_params pe_params = {pe_ctx_size, nullptr, true};
        state.pe_ctx = ggml_init(pe_params);
        ...
        for (int i = 0; i < n_fpn; ++i) {
            int H_pe = (int)state.neck_trk[i]->ne[2];
            int W_pe = (int)state.neck_trk[i]->ne[1];
            state.neck_trk_pe[i] = ggml_new_tensor_4d(state.pe_ctx, GGML_TYPE_F32,
                                                        hp.neck_dim, W_pe, H_pe, 1);
            char name[64];
            snprintf(name, sizeof(name), "neck_trk_pe_%d", i);
            ggml_set_name(state.neck_trk_pe[i], name);
        }
        ...
        for (int i = 0; i < n_fpn; ++i) {
            int H_pe = (int)state.neck_trk[i]->ne[2];
            int W_pe = (int)state.neck_trk[i]->ne[1];
            auto pe = sam3_sinusoidal_pe_2d(H_pe, W_pe, hp.neck_dim);
            ggml_backend_tensor_set(state.neck_trk_pe[i], pe.data(), 0, pe.size() * sizeof(float));
        }
    }

    SAM3_PROFILE_END(data_readback);
...
bool sam3_encode_image(sam3_state& state,
                       const sam3_model& model,
                       const sam3_image& image) {
    SAM3_PROFILE_BEGIN(encode_image);

    // ── EdgeTAM dispatch ─────────────────────────────────────────────────
    if (model.hparams.is_edgetam()) {
        bool ok = edgetam_encode_image(state, model, image);
        SAM3_PROFILE_END(encode_image);
        SAM3_PROFILE_PRINT();
        return ok;
    }
    ...
    if (model.hparams.is_sam2()) {
        bool ok = sam2_encode_image_hiera(state, model, image);
        SAM3_PROFILE_END(encode_image);
        SAM3_PROFILE_PRINT();
        return ok;
    }
    ...
    // Cache PE across frames — sinusoidal PE depends only on spatial dims + neck_dim,
    // which never change for the same model. Skip recomputation on subsequent frames.
    if (!state.pe_buf) {
...
static struct ggml_tensor * sam3_build_vit_attn_core_from_qkv(struct ggml_context * ctx,
    ...
    struct ggml_tensor * attn_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
    ...
    struct ggml_tensor* attn_mask = nullptr)  // [N_kv, N_q] or nullptr
{
    ...
    float scale = 1.0f / sqrtf((float)HD);
    auto* attn_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, attn_mask, scale);
    ...
    struct ggml_tensor* img_pe)     // [D, N_img, 1] sinusoidal PE
{
    ...
    for (int i = 0; i < (int)ge.layers.size(); ++i) {
        ...
        // 1. Self-attention (pre-norm, pos_enc_at_attn=False)
        {
            ...
            float scale = 1.0f / sqrtf((float)HD);
            auto* sa_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
        ...
        // 2. Cross-attention (pre-norm, Q from x, K from img+PE, V from img)
        {
            ...
            float scale = 1.0f / sqrtf((float)HD);
            auto* ca_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
...
static struct ggml_tensor* sam3_fenc_layer_forward(
    ...
    // ca_q_w stores fused [D, 3*D] weights split as Q-proj, K-proj, V-proj.
    {
        ...
        float scale = 1.0f / sqrtf((float)HD);
        auto* ca_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, ca_mask, scale);
...
static struct ggml_tensor* sam3_ddec_layer_forward(
    ...
    // 2. Cross-attention to text tokens (post-norm)
    {
        ...
        float scale = 1.0f / sqrtf((float)HD);
        auto* ca_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, text_mask, scale);
        ca_out = ggml_reshape_3d(ctx, ca_out, D, N_q, B);
    ...
    // 3. Cross-attention to conditioned image features (post-norm)
    {
        ...
        if (rpb_mask) {
            ...
            ca_out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
        }
    ...
    struct ggml_tensor* text_valid_mask)  // [T, 1, B] or nullptr
{
    ...
    if (text_valid_mask) {
        ...
        // The mask is pre-scaled to T / n_valid, so a plain mean over the token axis
        // matches sum(valid_tokens) / n_valid without relying on unsupported POOL_1D.
        auto* pooled_t = ggml_mean(ctx, tp);  // [1, D, B]
...
sam3_result sam3_segment_pcs(sam3_state& state,
    ...
    ggml_backend_t pcs_backend = model.backend;
...
static struct ggml_tensor* sam3_sam_attention(
    ...
    // Attention
    float scale = 1.0f / sqrtf((float)HD);
    auto* out = sam3_flash_attn_or_sdpa(ctx, Q, K, V, nullptr, scale);
...
sam3_result sam3_segment_pvs(sam3_state& state,
                             const sam3_model& model,
                             const sam3_pvs_params& params) {
    SAM3_PROFILE_BEGIN(segment_pvs);
#if SAM3_LOG_LEVEL >= 1
    auto t_start = std::chrono::high_resolution_clock::now();
#endif
    const auto& hp = model.hparams;
    ...
    // image_feats = neck_trk[2] + no_mem_embed (computed on CPU)
    {
        ...
        // Device-to-device copy when on GPU (avoids GPU→CPU→GPU roundtrip)
...
static sam3_prop_output sam3_propagate_single(
    ...
    // Create as 4D [D,H,H,1] matching neck_trk layout for device-to-device copy,
    // then reshape to 3D [D,N,1] inside the graph.
    auto* curr_4d = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32, D, H, H, 1);
    ggml_set_name(curr_4d, "prop_curr_4d");
...
sam3_result sam3_track_frame(sam3_tracker& tracker, sam3_state& state,
    ...
    SAM3_PROFILE_BEGIN(track_frame);
    if (model.hparams.visual_only) {
...
sam3_result sam3_propagate_frame(
        ...
        const sam3_model& model, const sam3_image& frame) {
    SAM3_PROFILE_BEGIN(propagate_frame);
    sam3_result result;
    const int D = model.hparams.neck_dim;
    if (!sam3_encode_image(state, model, frame)) return result;
    int fi = tracker.frame_index;
    ...
    SAM3_LOG(2, "%s: frame %d done — %zu tracked\n",
             __func__, fi, result.detections.size());
    SAM3_PROFILE_END(propagate_frame);
    SAM3_PROFILE_PRINT();
    return result;
}
...
sam3_image sam3_decode_video_frame(const std::string& video_path, int frame_index) {
    sam3_image img;
    sam3_video_info vinfo = sam3_get_video_info(video_path);
    if (vinfo.width <= 0) return img;
...
sam3_video_info sam3_get_video_info(const std::string& video_path) {
    ...
    auto find_val = [&](const std::string& key) {
        ...
        pos = output.find(":", pos + key.length());
        if (pos == std::string::npos) return std::string("");
        pos = output.find_first_not_of(" \t\":", pos + 1);
        if (pos == std::string::npos) return std::string("");
        size_t end = output.find_first_of(",}\n ]", pos);
        std::string val = output.substr(pos, end - pos);
        if (val.size() > 0 && val.front() == '"') {
            size_t q_end = val.find('"', 1);
            if (q_end != std::string::npos) val = val.substr(1, q_end - 1);
        }
        return val;
    };

    int w = atoi(find_val("width").c_str());
    int h = atoi(find_val("height").c_str());
    std::string fps_str = find_val("r_frame_rate");
    int nf = atoi(find_val("nb_frames").c_str());
    int rot = atoi(find_val("rotation").c_str());
    ...
    if (w > 0 && h > 0) {
        ...
        if (slash != std::string::npos) {
            float num = atof(fps_str.substr(0, slash).c_str());
            float den = atof(fps_str.substr(slash + 1).c_str());
            info.fps = (den > 0) ? num / den : 0.0f;
        }

        if (nf <= 0) {
            float duration = atof(find_val("duration").c_str());
            nf = (int)(duration * info.fps);
        }
        info.n_frames = nf;
    }
